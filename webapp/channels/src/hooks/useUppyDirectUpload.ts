...
export function useUppyDirectUpload(
	...
	// Updated synchronously during render (not in useEffect) so there is never
	// a window where getUploadParameters fires with a stale channelId.
	...
	if (!uppyRef.current) {
		...
		uppy.use(AwsS3, {
			...
			async getUploadParameters(file) {
				// Sanitize the filename so the S3 object key contains no
				// characters (spaces, etc.) that browsers will percent-encode
				// differently from what the server signed.  A mismatch between
				// the signed URL path and the actual request URL causes a
				// SignatureDoesNotMatch 403 from S3/DO Spaces.
				const rawName = (file as { name?: string }).name ?? 'upload';
