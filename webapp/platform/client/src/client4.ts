...
export default class Client4 {
	...
	// Upload Session Routes (chunked / resumable uploads, /api/v4/uploads)
	...
	/**
	 * Create a new upload session for a chunked / resumable file upload.
	 * Returns an UploadSession with a file_offset of 0 and the assigned id.
	 */
	createUploadSession = (session: { channel_id: string; filename: string; file_size: number }) => {
		return this.doFetch<{ id: string; channel_id: string; filename: string; file_size: number; file_offset: number }>(
			this.getUploadsRoute(),
			{ method: 'post', body: JSON.stringify({ ...session, type: 'attachment' }) },
		);
	};

	/**
	 * Retrieve a previously created upload session (useful for resuming).
	 */
	...
	/**
	 * Upload a chunk of data for an existing upload session.
	 * – Returns null when the server needs more data.
	 * – Returns a FileInfo when the upload is complete.
	 *
	 * @param uploadId  The upload session id.
	 * @param data      The raw Blob/ArrayBuffer chunk to send.
	 * @param offset    The byte offset of this chunk (for the Content-Range header).
	 * @param totalSize Total file size in bytes.
	 */
	uploadChunk = (uploadId: string, data: Blob, offset: number, totalSize: number) => {
		const opts = this.getOptions({ method: 'POST' });
		const headers: Record<string, string> = {
			...(opts.headers as Record<string, string>),
			'Content-Type': 'application/octet-stream',
			'Content-Range': `bytes ${offset}-${offset + data.size - 1}/${totalSize}`,
		};

		// We intentionally bypass doFetch here so we can set binary headers
		// and get back a fetch Response we can inspect for 204 vs 201.
		return fetch(this.getUploadRoute(uploadId), {
			method: 'POST',
			headers,
