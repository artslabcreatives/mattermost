...
func getCpaGroupID(app *App) (string, error) {
	...
	if appErr != nil {
		return "", errors.Wrap(appErr, "cannot register Custom Profile Attributes property group")
	}
...
func (a *App) GetCPAField(callerId, fieldID string) (*model.CPAField, *model.AppError) {
	...
	if appErr != nil {
		switch {
		case appErr.StatusCode == http.StatusNotFound:
...
func (a *App) CreateCPAField(callerId string, field *model.CPAField) (*model.CPAField, *model.AppError) {
	...
	if appErr != nil {
		return nil, model.NewAppError("CreateCPAField", "app.custom_profile_attributes.create_property_field.app_error", nil, "", http.StatusInternalServerError).Wrap(appErr)
	}
