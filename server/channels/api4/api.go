...
type Routes struct {
	...
	Typesense     *mux.Router // 'api/v4/typesense'
...
func Init(srv *app.Server) (*API, error) {
	...
	api.BaseRoutes.File = api.BaseRoutes.Files.PathPrefix("/{file_id:[A-Za-z0-9]{26}}").Subrouter()
	...
	api.BaseRoutes.PublicFile = api.BaseRoutes.Root.PathPrefix("/files/{file_id:[A-Za-z0-9]{26}}/public").Subrouter()
	...
	api.BaseRoutes.Typesense = api.BaseRoutes.APIRoot.PathPrefix("/typesense").Subrouter()
